package com.jayden.study.basicspringkotlin.repository
...
class CustomerRepository(private val template: ReactiveMongoTemplate) {
    ...
    companion object {
        val initialCustomers = listOf(
            Customer(1, "Kotlin", Customer.Telephone("+82", "12345678")),
            Customer(2, "Java"),
            Customer(3, "Javascript", Customer.Telephone("+44", "8765432")))
    }
    ...
    fun create(customer: Mono<Customer>) =
        template.save(customer)

    fun findById(id: Int) =
        template.findById<Customer>(id)

    fun deleteById(id: Int) =
        template.remove(Query(where("_id").isEqualTo(id)))
